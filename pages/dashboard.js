...
export default class Dashboard extends PureComponent {
  ...
  static async getInitialProps() {
    ...
    var permissionGrantedCount = await record.methods
      .getPermissionGrantedCount()
      .call();
    ...
    for (let i = 0; i < allPatients.length; i++) {
      ...
      var unixDate = await record.methods
    ...
    for (let i = 0; i < allPatients.length; i++) {
      var addr = allPatients[i];
      var count = await record.methods
    ...
    for (let i = 0; i < 12; i++) {
      if (monthName[mm] == Object.entries(dict)[i][0]) {
        if (i > 0 && Object.entries(dict)[i - 1] !== undefined) {
          resultPat =
            parseInt(Object.entries(dict)[i][1]) -
            parseInt(Object.entries(dict)[i - 1][1]);
          patientMonthOverMonthChange = (resultPat * 100).toFixed(2);
        } else {
          console.log("Previous month data is not available");
        }
        break;
      }
    }
    ...
    for (let i = 0; i < 12; i++) {
      if (monthName[mm] == Object.entries(apptdict)[i][0]) {
        if (i > 0 && Object.entries(apptdict)[i - 1] !== undefined) {
          resultAppt =
            parseInt(Object.entries(apptdict)[i][1]) -
            parseInt(Object.entries(apptdict)[i - 1][1]);
          ...
          <Card.Group centered itemsPerRow="2">
            <Card color={this.props.patGrowthColor}>
              <Card.Content>
                ...
                />
                <Card.Header>week-over Patient Growth</Card.Header>
              </Card.Content>
              <Card.Content extra>
                <Statistic size="small" color={this.props.patGrowthColor}>
                  <Statistic.Value>
                    {this.props.patientMonthOverMonthChange}%
                  </Statistic.Value>
                </Statistic>
              </Card.Content>
            </Card>
          ...
          <Card.Group centered itemsPerRow="4">
            ...
            <Card>
              ...
              <Card.Content extra>
                <Statistic size="small">
                  <Statistic.Value>
                    {this.props.permissionGrantedCount}
                  </Statistic.Value>
                </Statistic>
              </Card.Content>
            </Card>
